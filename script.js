...
async function loadDestinations(){
  ...
  <i class="fas fa-heart"></i> Add to Trip
  </button>
...
function recommend(){
  ...
  fetch(API + "/destinations")
    ...
    .then(data => {
      ...
      const filtered = data.filter(d => d.cost <= budget);
...
async function loadCart(){
  ...
  if(cart.length === 0){
  box.innerHTML = "<h3>🛒 Your trip is empty</h3><p>Start adding destinations ✈️</p>";
  ...
  data.filter(d => cart.includes(d.id)).forEach(d => {
    total += Number(d.cost);
